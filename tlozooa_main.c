...
static const GBGameAssets GB_TLOZOOA_GAME = {
    .game_id = "tlozooa",
    .rom_filename = "roms/tlozooa.gbc",
    .rom_data = tlozooa__rom_data,
    .rom_size = 1048576u,
    .expected_sha1 = { 0x88, 0x03, 0x74, 0xfb, 0x97, 0x8b, 0x18, 0xaf, 0x4a, 0xa5, 0x29, 0xe2, 0xe3, 0x2f, 0x7f, 0xfb, 0x4d, 0x7d, 0xd2, 0xf4 },
    .manifest = TLOZOOA_ASSETS_MANIFEST,
    .manifest_count = TLOZOOA_ASSETS_MANIFEST_COUNT,
};
...
int tlozooa_main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *tlozooa_default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "tlozooa");
        gb_context_set_save_id(interpreted_ctx, "tlozooa");
        tlozooa_init(generated_ctx);
        tlozooa_init(interpreted_ctx);
    ...
    gb_context_set_save_id(ctx, "tlozooa");
    ...
    if (!gb_chdir_to_exe_dir()) {
        fprintf(stderr, "[tlozooa] chdir to exe dir failed\n");
        return 1;
    }
    if (!gb_load_assets(&GB_TLOZOOA_GAME)) {
        fprintf(stderr, "[tlozooa] failed to load ROM/assets\n");
        return 1;
    }
#ifdef GB_HAS_SDL2
    gb_platform_set_game_id(ctx, GB_TLOZOOA_GAME.game_id);
#endif
    tlozooa_init(ctx);
    int exit_code = 0;
